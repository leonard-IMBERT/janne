...
class MockModel(IModel[MockInternalRepresentation]):
  ...
  def __init__(self) -> None:
    self._config = Optional[MockModelConf]
  ...
  def transform(self, raw_data: MockInternalRepresentation) -> MockInternalRepresentation:
    if self._config is None:
      raise RuntimeError("MockModel hasn't been initialized")
    return raw_data.apply(lambda d: d[0:10, :])
  ...
  def loss(self, prediction: MockInternalRepresentation, truth: NDArray) -> MockInternalRepresentation:
    if self._config is None:
      raise RuntimeError("MockModel hasn't been initialized")
